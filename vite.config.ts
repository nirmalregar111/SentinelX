...
/**
 * Exposes `GET /api/tunnel-url` in dev mode.
 * Starts localtunnel pointed at the Vite server port and returns the public URL.
 * The phone opens this URL to access camera-sender.html without Figma auth.
 */
function figmaPublicTunnelPlugin(): Plugin {
  let tunnelUrl: string | null = null
  let tunnelError: string | null = null

  async function ensureTunnel(vitePort: number) {
    if (tunnelUrl) return
    try {
      const lt = await import('localtunnel')
      const tunnel = await lt.default({ port: vitePort })
      tunnelUrl = tunnel.url as string
  ...
  return {
    name: 'figma-public-tunnel',
    apply: 'serve',
    configureServer(server) {
      const port = parseInt(process.env.PORT || '3000')
      // Start tunnel eagerly so it's ready when the user opens the phone camera flow
      ensureTunnel(port)

      server.middlewares.use((req: IncomingMessage, res: ServerResponse, next: () => void) => {
        if (req.url?.split('?')[0] !== '/api/tunnel-url') return next()
        ensureTunnel(port).then(() => {
          ...
          if (tunnelUrl) {
            ...
            res.statusCode = 503
            res.end(JSON.stringify({ error: tunnelError ?? 'Tunnel not ready' }))
          }
        })
      })
    },
